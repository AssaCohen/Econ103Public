...
In this exercise you'll calculate the *correlations* between different sets of returns.
  ...
  2. Download price data for Microsoft stock (...) over the same period used for Apple and IBM and construct daily log returns using closing prices.
  3. Calculate the correlation between Apple and Microsoft returns, and Microsoft and IBM returns.
...
microsoft.prices = ...
microsoft.returns = diff(log(microsoft.prices$Close))
cor(microsoft.returns, apple.returns)
cor(apple.returns, ibm.returns)
cor(microsoft.returns, ibm.returns)
```
